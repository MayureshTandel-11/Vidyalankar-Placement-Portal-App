...
/**
 * Recruitment stages a student may still receive updates for.
 */
function getEligibleStagesForStudent(opportunity, studentId, attendanceRecords = []) {
  const sid = normalizeStudentId(studentId);
...
/**
 * Student is still in the placement process (not absent / eliminated).
 */
function isStudentActiveInProcess(opportunity, studentId, attendanceRecords = []) {
  const sid = normalizeStudentId(studentId);
...
function filterActiveStagesForStudent(activeStages, opportunity, studentId, attendanceRecords = []) {
...
/**
 * Whether a student may see a timeline entry (including stage activations).
 */
function canStudentSeeTimelineEntry(entry, opportunity, studentId, attendanceRecords = []) {
  const sid = normalizeStudentId(studentId);
  ...
  if (entry.isStageActivation) {
    return eligible.has(stage);
  }

  if (RECRUITMENT_STAGE_ORDER.includes(stage)) {
    return eligible.has(stage);
  }
...
module.exports = {
  ...
  NON_RECRUITMENT_STAGES,
  getEligibleStagesForStudent,
  isStudentActiveInProcess,
