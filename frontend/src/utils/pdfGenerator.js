...
export const generateStudentProfilePDF = (student, enrollmentNumber) => {
  try {
    ...
    const basicInfo = [
      ...
      ["Student ID", enrollmentNumber || student.studentId || "N/A"],
      ["Email", student.email || "N/A"],
      ["Department", student.department || "N/A"],
      ["Class Year", student.year || "N/A"],
    ...
    if (student.academicInfo || student.year) {
      ...
      const academicInfo = [
        ["Class Year", student.year || "N/A"],
        ["Academic year (numeric)", student.academicInfo?.year ?? "—"],
        ["SSC %", student.academicInfo?.sscPercentage ?? "—"],
        ["HSC / Diploma %", student.academicInfo?.hscPercentage ?? "—"],
        ["CGPA", student.academicInfo?.cgpa ?? "—"],
        ["Semester performance", student.academicInfo?.semesterPerformance || "—"],
      ];
    ...
    if (student.projects && student.projects.length > 0) {
      ...
      const projectData = student.projects.map((proj) => [
        proj.title || "N/A",
        (proj.description || "N/A").slice(0, 220),
        (proj.technologies && proj.technologies.length ? proj.technologies.join(", ") : "—"),
        proj.link || "—",
      ]);
    ...
    if (resumeUrl) {
      ...
      doc.setFontSize(14);
      doc.setTextColor(...primaryColor);
      doc.text("Resume", 15, yPosition);
      yPosition += 7;
      doc.setFontSize(11);
      doc.setTextColor(79, 70, 229);
      const lines = doc.splitTextToSize(String(resumeUrl), 180);
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.setFontSize(9);
      doc.setTextColor(100, 100, 100);
      doc.text("Generated by Vidyalankar Placement Portal", doc.internal.pageSize.getWidth() / 2, doc.internal.pageSize.getHeight() - 14, {
        align: "center",
      });
      doc.setFontSize(10);
